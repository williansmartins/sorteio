...
$(window).scroll(function(){
	...
	setTimeout(reposicionarFaixas, 500);
...
$(document).ready(function() {	
	...
	$(".social-wrapper a").on('click', function(e) {
		ga('send', $(this).attr("data-link"));
	});
	...
	if($(window).width() > 767) {
		...
		redimencionarFaixas(200);
		console.info(1);
	}else{
		console.info(2);
		redimencionarFaixas(130);
	}
...
function reposicionarFaixas(){
	//apresentarRibbonInternacionalizacao
	if( $('.menu').css('display') == "block" ){
		var alturaDoMenu = $('.menu').css('height');
		$('.ribbon-internacionalizacao').stop().animate({top: alturaDoMenu}, 100);
	}else{
		$('.ribbon-internacionalizacao').stop().animate({top: "0px"}, 100);
	}
}
function redimencionarFaixas(tamanho){
	$('.ribbon-internacionalizacao').css('background-size','Auto ' + tamanho + 'px');
	$('.ribbon-internacionalizacao').css('width', ''+ tamanho + 'px');
	$('.ribbon-internacionalizacao').css('height', ''+ tamanho + 'px');
}
